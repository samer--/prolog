...
pldb:typed_value(T,V) --> {is_geometric(T)}, !, "'",geo(T,V),"'".

pldb:type_decode(F,X,P) :- is_geometric(F), !, atom_codes(X,C), phrase(geo(F,P),C,[]). 

geo( point,   point(A,B))         --> paren((pldb:float(A), comma, pldb:float(B))).
...
geo( circle,  circle(P,R))        --> "<", geo(point,P), comma, pldb:float(R), ">".
